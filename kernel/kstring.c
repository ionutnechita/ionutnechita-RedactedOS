...
kstring kstring_l(const char *literal){
    ...
    if (literal == NULL) return (kstring){.data = NULL, .length = 0};
    
    uint32_t len = compute_length(literal, 0);
    char *buf = (char*)talloc(len + 1);
    if (!buf) return (kstring){ .data = NULL, .length = 0};
...
kstring kstring_tail(const char *array, uint32_t max_length){
    if (array == NULL) return (kstring){.data = NULL, .length = 0};
...
kstring kstring_format_args(const char *fmt, const uint64_t *args, uint32_t arg_count){
    if(fmt == NULL) return (kstring){ .data = NULL, .length = 0 };
    
    if(arg_count > 0 && args == NULL) return (kstring){ .data = NULL, .length = 0 };
