...
In order to run the OS, you'll need to create a folder called fs inside the project's root directory. This folder will contain user processes. It must have the following subfolders: /redos/user/, with user processes placed inside with the .elf extension. The `user` process will automatically be placed in that folder when compiling. A script called `createfs` will run automatically to create an image from the fs folder. This script currently only works on MacOS host systems, it will need to be adapted to other systems.
Finally, the ./run script uses the unsigned compiled version of QEMU, which it expects to find in a folder named qemu parallel to the current project's directory. Replace this with the executable for QEMU installed on your machine.
